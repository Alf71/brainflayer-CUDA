...
__global__ KERNEL_LAUNCH_BOUNDS void workerPRIV(bool* isResult, bool* buffResult, char* __restrict__ lines, const uint32_t* __restrict__ indexes, const uint32_t indexes_size, const secp256k1_ge_storage* __restrict__ precPtr, const size_t precPitch, uint64_t round) {
	...
	char* stringStart;
	uint64_t len = 0;
	__align__(16) unsigned char pubKeys[THREAD_STEPS * 65];
	__align__(16) unsigned char prvKeys[THREAD_STEPS * 32];
	__align__(16) unsigned char pubKeysED[THREAD_STEPS * 32];
	...
	while (privKeyIx < THREAD_STEPS && starter < indexes_size) {
		...
		if (IS_HEX_DEV)
		{
			unsigned char toHash2[32];
			unhex((unsigned char*)toHash, 64, toHash2, 32);
			memcpy(&prvKeys[pk4], toHash2, 32);
		}
		else
		{
			memcpy(&prvKeys[pk4], toHash, 32);
		}
		privKeyIx++;
		starter++;
	}
	...
	for (uint64_t i = 0; i <= 2 * round; ++i)
	{
		...
		if (secp256_dev)
		{
			...
			int keyLenSkip;
			int pkField = 0;
			__align__(4) uint8_t tap_hash[32 * THREAD_STEPS];
			if (taproot)
			{
				TweakTaproot_batch(&tap_hash[0], &pubKeys[0], privKeyIx, precPtr, precPitch);
			}
			for (pkField = 0; pkField < THREAD_STEPS && pkField < privKeyIx; pkField++) {
				...
				if (icp)
				{
					...
					icp_principal_from_secp256k1(&pubKeys[pkField * 65], &principal[0]);
					uint8_t hash_addr[32];
		...
		if (ed25519_dev)
		{
			...
			for (int pkField = 0; pkField < THREAD_STEPS && pkField < privKeyIx; pkField++)
			{
				int pk4 = pkField * 32;
