...
__global__ void workerBrain(bool* isResult, bool* buffResult, char* __restrict__ lines, const uint32_t* __restrict__ indexes, const uint32_t indexes_size, const secp256k1_ge_storage* __restrict__ precPtr, const size_t precPitch, uint64_t round, uint8_t brain_mode, const uint32_t* __restrict__ iterations, const uint32_t iterations_size) {
	...
	uint64_t starter;
	char* toHashBuffer;
	uint32_t lens[THREAD_STEPS_BRAIN];
	...
	starter = THREAD_STEPS_BRAIN * tIx;

	while (privKeyIx < THREAD_STEPS_BRAIN && starter < indexes_size) {
		if (privKeyIx == 0) {
			if (starter == 0) {
				lens[privKeyIx] = indexes[0];
			}
			else {
				lens[privKeyIx] = indexes[starter] - indexes[starter - 1];
			}
		}
		else {
			lens[privKeyIx] = indexes[starter] - indexes[starter - 1];
	...
	for (uint32_t num = 0; num < iterations_size; num++)
	{
		...
		uint32_t ITERATION = iterations[num];
		
		for (int n = 0; n < privKeyIx; n++)
		{

			if (starter == 0) {
				toHashBuffer = (char*)lines;
			}
			else {
				toHashBuffer = ((char*)lines + indexes[starter - 1]);
			}
			starter++;
			...
			{
			case 1:
				sha3_256(&toHashBuffer[0], len, &prvKeys[pk4]);
				...
				break;
			case 2:
				keccak(&toHashBuffer[0], len, &prvKeys[pk4], 32);
				for (int i = 1; i < ITERATION; i++)
				{
				...
				break;
			case 3:
				Blake2b_256((uint8_t*)&toHashBuffer[0], len, &prvKeys[pk4]);
				for (int i = 1; i < ITERATION; i++)
				{
...
#pragma unroll 32
				...
				{
					if (len >= 32)
						seed[i] = toHashBuffer[(len - 32) + i];
					else
						seed[i] = (i < (32 - (int)len)) ? 0 : toHashBuffer[i - (32 - (int)len)];
				}
				memcpy(&prvKeys[pk4], &seed[0], 32);
				break;
			default:
				SHA256((uint8_t*)&toHashBuffer[0], len, &prvKeys[pk4]);
				for (int i = 1; i < ITERATION; i++)
				{
					...
					if (uncompressed) {
						...
						if (checkHash(hash160)) {
							...
							if (idx < MAX_FOUNDS_DEV) {
								memcpy(d_foundStrings[idx], &toHashBuffer[0], len);
