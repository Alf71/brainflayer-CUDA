...
public:
    ...
    __device__ __host__ static inline unsigned char addcarry_u64(
        unsigned char Carry,
    ...
    __device__ static inline uint128_t mul128(uint64_t x, uint64_t y)
    {
        uint128_t res;
#ifdef __CUDA_ARCH__
        res.m_lo = x * y;
...
__device__ __host__ static inline uint128_t DivMod(uint128_t N, uint128_t D, uint128_t& rem)
{
    if (D > N)
    {
        rem = N;
