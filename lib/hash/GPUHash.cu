...
__device__   void _GetHash160(const unsigned char* __restrict__  pubkey, int& keyLen, uint8_t* __restrict__ hash) {
    ...
    __align__(16) uint32_t publicKeyBytes[32];
    __align__(16) uint32_t s[16];
...
__device__   void _GetHash160Comp(unsigned char* pubkey, int& keyLen, uint8_t* hash) {
    int keyLenSkip = keyLen;
    __align__(16) uint32_t publicKeyBytes[16];
    __align__(16) uint32_t s[16];
...
__device__  void _GetHash160P2SHCompFromHash(uint32_t* h, uint32_t* hash) {
    __align__(16) uint32_t scriptBytes[16];
    __align__(16) uint32_t s[16];
...
#define GPUHASH_TH_ELT(t, c0, c1, c2, c3, c4, d0, d1, d2, d3, d4) \
    t = rol((uint64_t)(d0 ^ d1 ^ d2 ^ d3 ^ d4), 1) ^ (c0 ^ c1 ^ c2 ^ c3 ^ c4)
#define GPUHASH_THETA(s00, s01, s02, s03, s04, s10, s11, s12, s13, s14, s20, s21, s22, s23, s24, s30, s31, s32, s33, s34, s40, s41, s42, s43, s44) \
    GPUHASH_TH_ELT(t0, s40, s41, s42, s43, s44, s10, s11, s12, s13, s14); \
    GPUHASH_TH_ELT(t1, s00, s01, s02, s03, s04, s20, s21, s22, s23, s24); \
    GPUHASH_TH_ELT(t2, s10, s11, s12, s13, s14, s30, s31, s32, s33, s34); \
    GPUHASH_TH_ELT(t3, s20, s21, s22, s23, s24, s40, s41, s42, s43, s44); \
    GPUHASH_TH_ELT(t4, s30, s31, s32, s33, s34, s00, s01, s02, s03, s04); \
...
#define GPUHASH_RHOPI(s00, s01, s02, s03, s04, s10, s11, s12, s13, s14, s20, s21, s22, s23, s24, s30, s31, s32, s33, s34, s40, s41, s42, s43, s44) \
    t0 = rol(s10, 1);  s10 = rol(s11,44); s11 = rol(s41,20); s41 = rol(s24,61); \
    s24 = rol(s42,39); s42 = rol(s04,18); s04 = rol(s20,62); s20 = rol(s22,43); \
    s22 = rol(s32,25); s32 = rol(s43, 8); s43 = rol(s34,56); s34 = rol(s03,41); \
    s03 = rol(s40,27); s40 = rol(s44,14); s44 = rol(s14, 2); s14 = rol(s31,55); \
    s31 = rol(s13,45); s13 = rol(s01,36); s01 = rol(s30,28); s30 = rol(s33,21); \
    s33 = rol(s23,15); s23 = rol(s12,10); s12 = rol(s21, 6); s21 = rol(s02, 3); s02 = t0
#define GPUHASH_KHI5(a,b,c,d,e) t0 = a ^ (~b & c); t1 = b ^ (~c & d); t2 = c ^ (~d & e); t3 = d ^ (~e & a); t4 = e ^ (~a & b); a = t0; b = t1; c = t2; d = t3; e = t4
#define GPUHASH_KHI(s00, s01, s02, s03, s04, s10, s11, s12, s13, s14, s20, s21, s22, s23, s24, s30, s31, s32, s33, s34, s40, s41, s42, s43, s44) \
    GPUHASH_KHI5(s00,s10,s20,s30,s40); GPUHASH_KHI5(s01,s11,s21,s31,s41); GPUHASH_KHI5(s02,s12,s22,s32,s42); GPUHASH_KHI5(s03,s13,s23,s33,s43); GPUHASH_KHI5(s04,s14,s24,s34,s44)
#pragma unroll 24
    for (int i = 0; i < 24; ++i) {
        GPUHASH_THETA(st[0], st[5], st[10], st[15], st[20], st[1], st[6], st[11], st[16], st[21], st[2], st[7], st[12], st[17], st[22], st[3], st[8], st[13], st[18], st[23], st[4], st[9], st[14], st[19], st[24]);
        GPUHASH_RHOPI(st[0], st[5], st[10], st[15], st[20], st[1], st[6], st[11], st[16], st[21], st[2], st[7], st[12], st[17], st[22], st[3], st[8], st[13], st[18], st[23], st[4], st[9], st[14], st[19], st[24]);
        GPUHASH_KHI(st[0], st[5], st[10], st[15], st[20], st[1], st[6], st[11], st[16], st[21], st[2], st[7], st[12], st[17], st[22], st[3], st[8], st[13], st[18], st[23], st[4], st[9], st[14], st[19], st[24]);
        st[0] ^= RC[i];
    }
#undef GPUHASH_TH_ELT
#undef GPUHASH_THETA
#undef GPUHASH_RHOPI
#undef GPUHASH_KHI5
#undef GPUHASH_KHI
...
__device__ __forceinline__ void keccak_eth64_fast(const uint8_t* __restrict__ message, unsigned char* __restrict__ output)
{
    union {
        uint8_t b[200];
        uint64_t q[25];
...
#pragma unroll
    for (int i = 0; i < 200; ++i) h.b[i] = 0;
#pragma unroll
    for (int i = 0; i < 64; ++i) h.b[i] = message[i];
    h.d[16] ^= 0x01u;
    h.d[33] ^= 0x80000000u;
    keccakf(h.q);
#pragma unroll
    for (int i = 0; i < 32; ++i) output[i] = h.b[i];
}

__device__ __forceinline__ uint64_t gpuhash_bswap64(uint64_t v)
{
    return ((uint64_t)bswap32((uint32_t)v) << 32) | (uint64_t)bswap32((uint32_t)(v >> 32));
}

__device__ __forceinline__ uint64_t gpuhash_load_le64(const uint8_t* __restrict__ p)
{
    uint64_t v = 0ull;
#pragma unroll
    for (int i = 0; i < 8; ++i) {
...
__device__ __forceinline__ void keccak_eth64_tail20_from_state(uint64_t* __restrict__ q, uint8_t* __restrict__ b, uint32_t* __restrict__ out5)
{
    q[8] = 0x01ull;
    q[16] = 0x8000000000000000ull;

    keccakf(q);

#pragma unroll
    for (int i = 0; i < 5; ++i) {
        const int hpos = 12 + i * 4;
        out5[i] = ((uint32_t)b[hpos]) |
            ((uint32_t)b[hpos + 1] << 8) |
            ((uint32_t)b[hpos + 2] << 16) |
            ((uint32_t)b[hpos + 3] << 24);
    }
}

__device__ void keccak_eth64_tail20_bytes(const uint8_t* __restrict__ message, uint32_t* __restrict__ out5)
{
    union {
        uint8_t b[200];
        uint64_t q[25];
...
#pragma unroll
    for (int i = 0; i < 25; ++i) h.q[i] = 0ull;

#pragma unroll
    for (int i = 0; i < 8; ++i) {
        h.q[i] = gpuhash_load_le64(message + i * 8);
    }

    keccak_eth64_tail20_from_state(h.q, h.b, out5);
}

__device__ void keccak_eth64_tail20_from_xy(const uint64_t* __restrict__ px, const uint64_t* __restrict__ py, uint32_t* __restrict__ out5)
{
    union {
        uint8_t b[200];
        uint64_t q[25];
...
#pragma unroll
    ...
    h.q[0] = gpuhash_bswap64(px[3]);
    h.q[1] = gpuhash_bswap64(px[2]);
    h.q[2] = gpuhash_bswap64(px[1]);
    h.q[3] = gpuhash_bswap64(px[0]);
    h.q[4] = gpuhash_bswap64(py[3]);
    h.q[5] = gpuhash_bswap64(py[2]);
    h.q[6] = gpuhash_bswap64(py[1]);
    h.q[7] = gpuhash_bswap64(py[0]);
...
{
    if (message_len == 64 && output_len == 32) {
        keccak_eth64_fast((const uint8_t*)message, output);
        return;
    }
...
__device__  void sha3_256_old(const uint8_t* in, size_t in_len, uint8_t* out) {
    ...
    __align__(16) uint8_t block[136];
    __align__(16) uint64_t state[25];
