...
// cuda_memcmp: performs cuda memcmp.
int cuda_memcmp(const void* lhs, const void* rhs, size_t n)
{
	if (n == 0) return 0;
	...
	while (n && (reinterpret_cast<uintptr_t>(p1) & 7)) {
		int diff = int(*p1) - int(*p2);
		if (diff) return diff;
		++p1; ++p2; --n;
	}

	const uint64_t* w1 = reinterpret_cast<const uint64_t*>(p1);
	const uint64_t* w2 = reinterpret_cast<const uint64_t*>(p2);
	while (n >= 8) {
		uint64_t a = *w1;
		uint64_t b = *w2;
		if (a != b) {
            uint64_t v = a ^ b;
            // __ffsll is 1-based; convert to 0-based byte index.
            int byte_pos = (int)((__ffsll((long long)v) - 1) >> 3);
            const uint8_t* ca = reinterpret_cast<const uint8_t*>(&a);
            const uint8_t* cb = reinterpret_cast<const uint8_t*>(&b);
            return int(ca[byte_pos]) - int(cb[byte_pos]);
		}
		++w1; ++w2; n -= 8;
	}

	p1 = reinterpret_cast<const uint8_t*>(w1);
	p2 = reinterpret_cast<const uint8_t*>(w2);
	while (n--) {
		int diff = int(*p1) - int(*p2);
		if (diff) return diff;
		++p1; ++p2;
...
//donna-23bit_impl
__device__  const uint32_t reduce_mask_25 = (1 << 25) - 1;
__device__  const uint32_t reduce_mask_26 = (1 << 26) - 1;

/* out = in */
__device__   void
// curve25519_copy: copies the related data for curve 25519.
...
/* out = a + b */
__device__   void
// curve25519_add: adds the related data for curve 25519.
...
__device__   void
// curve25519_add_after_basic: adds after basic for curve 25519.
...
__device__   void
// curve25519_add_reduce: adds reduce for curve 25519.
...
/* multiples of p */
__device__  const uint32_t twoP0 = 0x07ffffda;
__device__  const uint32_t twoP13579 = 0x03fffffe;
__device__  const uint32_t twoP2468 = 0x07fffffe;
__device__  const uint32_t fourP0 = 0x0fffffb4;
__device__  const uint32_t fourP13579 = 0x07fffffc;
__device__  const uint32_t fourP2468 = 0x0ffffffc;

/* out = a - b */
__device__   void
// curve25519_sub: subtracts the related data for curve 25519.
...
__device__   void
// curve25519_sub_after_basic: subtracts after basic for curve 25519.
...
__device__   void
// curve25519_sub_reduce: subtracts reduce for curve 25519.
...
/* out = -a */
__device__   void
// curve25519_neg: performs curve 25519 neg.
...
__device__  void
// curve25519_mul: multiplies the related data for curve 25519.
...
/* out = in*in */
__device__  void
// curve25519_square: performs curve 25519 square.
...
/* out = in ^ (2 * count) */
__device__  void
// curve25519_square_times: performs curve 25519 square times.
...
__device__  void
// curve25519_expand: expands the related data for curve 25519.
 ...
 */
__device__  void
// curve25519_contract: performs curve 25519 contract.
...
/* out = (flag) ? in : out */
__device__   void
// curve25519_move_conditional_bytes: performs curve 25519 move conditional bytes.
...
/* if (iswap) swap(a, b) */
__device__   void
// curve25519_swap_conditional: swaps conditional for curve 25519.
...
__device__  void
// curve25519_pow_two5mtwo0_two250mtwo0: computes two 5 mtwo 0 two 250 mtwo 0 for curve 25519.
 ...
 */
__device__  void
// curve25519_recip: performs curve 25519 recip.
 ...
 */
__device__  void
// curve25519_pow_two252m3: computes two 252 m 3 for curve 25519.
...
__device__ bool IS_SCALAR = false;
__device__ bool IS_HASH = false;
__device__ bool IS_LE = false;

// Device helper: set_scalar.
...
__device__ __forceinline__
// reverse: performs reverse.
uint8_t* reverse(uint8_t* p) {
#pragma unroll
	for (int i = 0; i < 16; ++i) {
		uint8_t t = p[i];
		p[i] = p[31 - i];
...
__device__  void
// ed25519_extsk: performs ed 25519 extsk.
...
__device__  void
// ed25519_hram: performs ed 25519 hram.
...
// ed25519_publickey_scalar: performs ed 25519 publickey scalar.
ed25519_publickey_scalar(const ed25519_secret_key sk, ed25519_public_key pk) {
	bignum256modm a;
	ge25519 ALIGN(16) A;
...
__device__ __forceinline__ void
// ed25519_publickey_scalar_hash: computes the related data for ed 25519 publickey scalar.
ed25519_publickey_scalar_hash(const ed25519_secret_key sk, ed25519_public_key pk) {
	bignum256modm a;
	ge25519 ALIGN(16) A;
	ed25519_secret_key extsk;
...
__device__ void
// ed25519_publickey: performs ed 25519 publickey.
...
__device__ __forceinline__ void
// ed25519_publickey_scalar_hash_batch: computes batch for ed 25519 publickey scalar.
ed25519_publickey_scalar_hash_batch(const uint8_t* __restrict__ sk, uint8_t* __restrict__ pk, int n) {
	bignum256modm a;
	ge25519 ALIGN(16) A;

#pragma unroll 1
	for (int i = 0; i < n; ++i) {
		const uint8_t* sk_i = sk + (i << 5);      // i*32
		uint8_t* pk_i = pk + (i << 5);

		ed25519_secret_key extsk;

		memcpy(extsk, sk_i, 32);
...
__device__ __forceinline__ void
// ed25519_publickey_scalar_batch: performs ed 25519 publickey scalar batch.
ed25519_publickey_scalar_batch(const uint8_t* __restrict__ sk, uint8_t* __restrict__ pk, int n) {
	bignum256modm a;
	ge25519 ALIGN(16) A;
	const bool is_le = IS_LE;
...
__device__ __forceinline__ void
// ed25519_publickey_batch: performs ed 25519 publickey batch.
ed25519_publickey_batch(const uint8_t* __restrict__ sk, uint8_t* __restrict__ pk, int n) {
	bignum256modm a;
	ge25519 ALIGN(16) A;
	hash_512bits extsk;
...
__device__ void
// ed25519_key_to_pub_batch: performs ed 25519 key to pub batch.
ed25519_key_to_pub_batch(const uint8_t* __restrict__ sk, uint8_t* __restrict__ pk, int n)
{
	const bool is_scalar = IS_SCALAR;
	const bool is_hash = IS_HASH;
	if (is_scalar)
	{

		ed25519_publickey_scalar_batch(&sk[0], &pk[0], n);
	}
	else if (is_hash)
	{
		ed25519_publickey_scalar_hash_batch(&sk[0], &pk[0], n);
	}
	else
	{
		ed25519_publickey_batch(&sk[0], &pk[0], n);

	}
}

__device__ void
// ed25519_key_to_pub: performs ed 25519 key to pub.
ed25519_key_to_pub(const ed25519_secret_key sk, ed25519_public_key pk)
{
	const bool is_scalar = IS_SCALAR;
	const bool is_hash = IS_HASH;
	if (is_scalar)
	{
		...
		ed25519_publickey_scalar(&sk[0], &pk[0]);
	}
	else if (is_hash)
	{
		ed25519_publickey_scalar_hash(&sk[0], &pk[0]);
	}
	else
	{
		ed25519_publickey(&sk[0], &pk[0]);
...
__device__ void
// ed25519_sign: performs ed 25519 sign.
...
__device__ int
// ed25519_sign_open: opens the related data for ed 25519 sign.
...
__device__ void
// curved25519_scalarmult_basepoint: performs curved 25519 scalarmult basepoint.
...
__device__  int
// ed25519_verify: checks whether ed 25519 is valid.
...
__device__  const ge25519 ALIGN(16) ge25519_basepoint = {
...
__device__  const bignum25519 ALIGN(16) ge25519_sqrtneg1 = {
	0x020ea0b0,0x0186c9d2,0x008f189d,0x0035697f,0x00bd0c60,0x01fbd7a7,0x02804c9e,0x01e16569,0x0004fc1d,0x00ae0c92
};

// Device helper: ALIGN.
__device__  const ge25519_niels ALIGN(16) ge25519_niels_sliding_multiples[32] = {
...
__device__  const uint8_t ALIGN(16) ge25519_niels_base_multiples[256][96] = {
...
__device__   void
// ge25519_p1p1_to_partial: performs ge 25519 p 1 p 1 to partial.
...
__device__   void
// ge25519_p1p1_to_full: performs ge 25519 p 1 p 1 to full.
...
__device__  void
// ge25519_full_to_pniels: performs ge 25519 full to pniels.
...
__device__  void
// ge25519_add_p1p1: adds p 1 p 1 for ge 25519.
...
__device__  void
// ge25519_double_p1p1: performs ge 25519 double p 1 p 1.
...
__device__  void
// ge25519_nielsadd2_p1p1: performs ge 25519 nielsadd 2 p 1 p 1.
...
__device__  void
// ge25519_pnielsadd_p1p1: performs ge 25519 pnielsadd p 1 p 1.
...
__device__  void
// ge25519_double_partial: performs ge 25519 double partial.
...
__device__  void
// ge25519_double: performs ge 25519 double.
...
__device__  void
// ge25519_add: adds the related data for ge 25519.
...
__device__  void
// ge25519_nielsadd2: performs ge 25519 nielsadd 2.
...
__device__  void
// ge25519_pnielsadd: performs ge 25519 pnielsadd.
...
__device__  void
// ge25519_pack: performs ge 25519 pack.
...
__device__  int
// ge25519_unpack_negative_vartime: performs ge 25519 unpack negative vartime.
...
__device__  void
// ge25519_double_scalarmult_vartime: performs ge 25519 double scalarmult vartime.
...
__device__  uint32_t
// ge25519_windowb_equal: checks whether ge 25519 windowb is valid.
ge25519_windowb_equal(uint32_t b, uint32_t c) {
	return ((b ^ c) - 1) >> 31;
}

__device__  void
// ge25519_scalarmult_base_choose_niels: selects niels for ge 25519 scalarmult base.
...
__device__  void
// ge25519_scalarmult_base_niels: performs ge 25519 scalarmult base niels.
...
__device__  const size_t limb128bits = (128 + bignum256modm_bits_per_limb - 1) / bignum256modm_bits_per_limb;
...
__device__  void
// heap_swap: swaps the related data for heap.
...
__device__  void
// heap_insert_next: performs heap insert next.
...
__device__  void
// heap_updated_root: performs heap updated root.
...
__device__  void
// heap_build: builds the related data for heap.
...
__device__  void
// heap_extend: performs heap extend.
...
__device__  void
// heap_get_top2: gets top 2 for heap.
...
/* */
__device__  void
// ge25519_multi_scalarmult_vartime_final: performs ge 25519 multi scalarmult vartime final.
...
/* count must be >= 5 */
__device__  void
// ge25519_multi_scalarmult_vartime: performs ge 25519 multi scalarmult vartime.
...
__device__  int
// ge25519_is_neutral_vartime: checks whether ge 25519 neutral vartime is valid.
ge25519_is_neutral_vartime(const ge25519* p) {
	...
	return (cuda_memcmp(point_buffer[0], zero, 32) == 0) && (cuda_memcmp(point_buffer[1], point_buffer[2], 32) == 0);
}
...
// lt_modm: checks whether modm is valid.
...
__device__  void
// reduce256_modm: performs reduce 256 modm.
...
*/
__device__  void
// barrett_reduce256_modm: performs barrett reduce 256 modm.
...
__device__  void
// neg256_modm: performs neg 256 modm.
...
/* subtraction x-y % m */
__device__  void
// sub256_modm: subtracts 256 modm.
...
// Device helper: is_reduced256_modm.
...
__device__  void
// mul256_modm: multiplies 256 modm.
...
__device__  void
// expand256_modm: expands 256 modm.
...
__device__  void
// expand_raw256_modm: expands raw 256 modm.
...
__device__  void
// contract256_modm: performs contract 256 modm.
...
__device__  void
// contract256_window4_modm: performs contract 256 window 4 modm.
...
__device__  void
// contract256_slidingwindow_modm: performs contract 256 slidingwindow modm.
...
__device__  void
// sub256_modm_batch: subtracts 256 modm batch.
...
/* is a < b */
__device__  int
// lt256_modm_batch: checks whether 256 modm batch is valid.
...
/* is a <= b */
__device__  int
// lte256_modm_batch: checks whether 256 modm batch is valid.
...
/* is a == 0 */
__device__  int
// iszero256_modm_batch: performs iszero 256 modm batch.
...
/* is a == 1 */
__device__  int
// isone256_modm_batch: performs isone 256 modm batch.
...
__device__  int
// isatmost128bits256_modm_batch: performs isatmost 128 bits 256 modm batch.
...
__device__
// add_modL_from_bytes: adds mod l from bytes.
void add_modL_from_bytes(uint8_t out32[32], const uint8_t inX[32], const uint8_t inY[32]) {
	bignum256modm X, Y, R;
	expand_raw256_modm(X, inX);
	expand_raw256_modm(Y, inY);
	add256_modm(R, X, Y);
	contract256_modm(out32, R);
}
